...
void Platform::create(GLuint program) {
  destroy();
...
void Platform::paint(const GameData &gameData) {
...
void Platform::update(GameData const &gameData, float deltaTime) {
  // Movimento na horizontal
  if (gameData.m_input[gsl::narrow<size_t>(Input::Left)] &&
      gameData.m_state == State::Playing) {
      auto const left{glm::rotate(glm::vec2{-1.0f, 0.0f}, m_rotation)};
      m_velocity += left * deltaTime;
      }
  if (gameData.m_input[gsl::narrow<size_t>(Input::Right)] &&
      gameData.m_state == State::Playing) {
      auto const right{glm::rotate(glm::vec2{+1.0f, 0.0f}, m_rotation)};
      m_velocity += right * deltaTime;
      }
}